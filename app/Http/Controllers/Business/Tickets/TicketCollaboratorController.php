...
{
    $user = Auth::user();
    $tickets = $user->tickets; // ou $user->tickets, se o relacionamento for plural

    return view('app.business.tickets.ticket_collaborator.ticket_collaborator_index', compact('user', 'tickets'));
}
...
{
    $user = Auth::User();
    $tickets = $user->tickets()->with('ticketCategory')->get();
